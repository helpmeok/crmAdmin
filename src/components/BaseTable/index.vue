      ...
      <template v-for="(item,prop) of newColumns">
        <el-table-column v-if="item && !item.isHide" :key="prop" :show-overflow-tooltip="item.showTooltip"
          ...
          <template #default="scope">
            ...
            <span v-else>
              <dict-tag v-if="item?.options?.length || item.dictType" :options="item.options" :optionLabelKey="item.optionLabelKey"
                        :optionValueKey="item.optionValueKey" :value="scope.row[prop]"></dict-tag>
              <span v-else>{{item.prefix}}{{ colStrEmptyFn(scope.row[prop]) }}{{item.suffix}}</span>
            </span>
          </template>
        </el-table-column>
      </template>
