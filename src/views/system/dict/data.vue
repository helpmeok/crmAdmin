  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
      <el-form-item label="字典名称" prop="dictType">
        <el-select v-model="queryParams.dictType" filterable style="width: 200px">
    ...
    <el-table v-loading="loading" v-autoHeight="{bottomHeight:80,isAutoHeight:true}" :data="dataList" @selection-change="handleSelectionChange">
      <!-- <el-table-column label="字典编码" align="center" prop="id" /> -->
      <el-table-column type="index" align="center" width="60" label="序号" />
      <el-table-column label="字典标签" align="center" prop="dictLabel">
        <template #default="scope">
          <!-- <span
                v-if="(scope.row.listClass == '' || scope.row.listClass == 'default') && (scope.row.cssClass == '' || scope.row.cssClass == null)">{{ scope.row.dictLabel }}</span>
...
import { listAllType, getType } from "@/api/system/dict/type";
import {
  listData,
  getData,
  delData,
...
const { SYS_STATUS } = inject("useDict")("SYS_STATUS");
const { SYS_YESORNO } = inject("useDict")("SYS_YESORNO");
...
function handleDelete(row) {
  const ids = row.id;
  proxy.$modal
    .confirm('是否确认删除字典标签为"' + row.dictLabel + '"的数据项？')
    .then(function () {
      return delData({ ids });
    })
    .then(() => {
